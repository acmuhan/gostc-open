...
import (
	...
	"server/service/normal/gost_client_forward"
)
...
func Admission(c *gin.Context) {
	...
	if err := svr.Admission(middleware.GetClaims(c), req); err != nil {
		bean.Response.Fail(c, err.Error())
		return
	}
	bean.Response.Ok(c)
...
func Matcher(c *gin.Context) {
	var req service.MatcherReq
	if err := c.ShouldBindJSON(&req); err != nil {
		bean.Response.Param(c, err)
		return
	}
	if err := svr.Matcher(middleware.GetClaims(c), req); err != nil {
		bean.Response.Fail(c, err.Error())
		return
	}
	bean.Response.Ok(c)
