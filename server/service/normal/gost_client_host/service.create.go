...
func (service *service) Create(claims jwt.Claims, req CreateReq) error {
	...
	return db.Transaction(func(tx *query.Query) error {
		...
		if _, err := commerce.PayPackage(tx, user, cfg.Amount, model.ORDER_BIZ_HOST_CREATE, "", model.Map{"package": commerce.SnapshotConfig(cfg), "nodeCode": node.Code, "clientCode": client.Code}, "购买套餐"); err != nil {
			return err
		}
