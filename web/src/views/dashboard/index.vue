...
const walletPageCount = computed(() => Math.max(1, Math.ceil(walletTotal.value / 10)))
...
const ledgerColumns = [
  ...
  { title: '积分', key: 'amount', render: r => h('span', { style: r.direction === 2 ? 'color:#d03050' : 'color:#18a058' }, `${r.direction === 2 ? '-' : '+'}${money(r.amount)}`) },
...
async function loadLedger() {
  ...
  try {
    const res = await apiWalletLedger({ page: walletPage.value, size: 10 })
...
async function redeem() {
  ...
  try {
    ...
    await loadWallet()
    await loadLedger()
    let res2 = await apiAuthUserInfo()
    state.value.userInfo = res2.data
    appStore().userInfo = res2.data
  } catch (_) {}
    ...
    <AppCard :show-border="false">
      <n-grid :cols="gridColsComputed" :x-gap="12" :y-gap="12">
        ...
        <n-grid-item :span="gridColsComputed">
          <n-el tag="div" :style="cardStyleComputed">
            <AppCard :show-border="false">
              <n-h4 style="font-weight: bold">钱包</n-h4>
              <n-descriptions :column="1" label-placement="left">
                <n-descriptions-item label="积分">
                  <span style="font-weight:700;font-size:18px">{{ money(wallet?.amount) }}</span>
                </n-descriptions-item>
              </n-descriptions>
              <n-grid :cols="gridColsComputed" :x-gap="12" :y-gap="12" style="margin-top:12px">
                <n-grid-item>
                  <n-card size="small" title="CDK 兑换">
                    <n-space>
                      <n-input v-model:value="cdk" placeholder="输入兑换码" style="width:200px;max-width:100%" />
                      <n-button type="primary" :loading="redeemLoading" @click="redeem">兑换</n-button>
                    </n-space>
                  </n-card>
                </n-grid-item>
                <n-grid-item>
                  <n-card size="small" title="积分充值">
                    <n-space>
                      <n-input-number v-model:value="rechargeAmount" :min="0.01" :precision="2" placeholder="充值金额" style="width:130px" />
                      <n-select v-model:value="rechargePayType" :options="payTypeOptions" placeholder="支付方式" style="width:120px" />
        ...
        <n-grid-item :span="gridColsComputed">
          <n-el tag="div" :style="cardStyleComputed">
            <AppCard :show-border="false">
              <n-h4 style="font-weight: bold">积分流水</n-h4>
              <n-data-table size="small" :columns="ledgerColumns" :data="ledgers" :loading="walletLoading" />
              <n-pagination style="justify-content:flex-end;margin-top:12px" v-model:page="walletPage" :page-count="walletPageCount" @update:page="loadLedger" />
            </AppCard>
          </n-el>
        </n-grid-item>
        <n-grid-item :span="gridColsComputed">
